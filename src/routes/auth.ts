...
async function authRoutes(fastify: FastifyInstance) {
  // Register User
  fastify.post<{ Reply: FReply }>("/register", async (request, reply) => {
  ...
  // Login User
  fastify.post<{ Reply: FReply }>("/login", async (request, reply) => {
    const { username, password } = request.body as {
      username: string;
      password: string;
    };
  ...
  // Save Progress
  fastify.post<{ Reply: FReply }>("/save-progress", async (request, reply) => {
    try {
      const authHeader = request.headers.authorization;
      if (!authHeader) {
        return reply.status(401).send({ message: "No token provided" });
  ...
  // Load Progress
  fastify.get<{ Reply: FReply }>("/load-progress", async (request, reply) => {
    try {
      const authHeader = request.headers.authorization;
      if (!authHeader) {
        return reply.status(401).send({ message: "No token provided" });
